		...
		public void Update() {
            ...
            if (mouse.X >= Left && mouse.X < Left + Width && mouse.Y >= Top && mouse.Y < Top + Height && System.Windows.Forms.Form.MouseButtons == System.Windows.Forms.MouseButtons.Right) {
                ...
                if (mouse.X < start.X) {
                    ...
                    Canvas.SetLeft(rect, start.X - Left);
                    hitbox.X = (float)start.X;
                }
                if (mouse.Y < start.Y) {
                    Canvas.SetTop(rect, mouse.Y - Top);
                    hitbox.Y = mouse.Y;
                } else {
                    Canvas.SetTop(rect, start.Y - Top);
                    hitbox.Y = (float)start.Y;
                }
                rect.Width = Math.Abs(mouse.X - start.X);
                rect.Height = Math.Abs(mouse.Y - start.Y);
